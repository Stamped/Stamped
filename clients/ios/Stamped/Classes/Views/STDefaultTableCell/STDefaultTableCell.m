...
@synthesize showsDisclosure=_showsDisclosure;
...
- (void)layoutSubviews {
    ...
    frame.size.width = MIN(floorf(self.bounds.size.width - 60.0f), frame.size.width);
...
- (void)setShowsDisclosure:(BOOL)showsDisclosure {
    ...
    if (_showsDisclosure) {
        ...
        if (_disclosureView) {
            [_disclosureView removeFromSuperview], _disclosureView=nil;
        }
