...
@interface STMetadataContainerView : STViewContainer

@property (nonatomic, readwrite, retain) CALayer* gradient;

@end

@implementation STMetadataContainerView
...
- (void)childView:(UIView *)view shouldChangeHeightBy:(CGFloat)delta overDuration:(CGFloat)seconds {
    [UIView animateWithDuration:seconds animations:^{
        self.gradient.frame = CGRectMake(0, 0, self.frame.size.width, self.frame.size.height + delta);
    }];
    [super childView:view shouldChangeHeightBy:delta overDuration:seconds];
}
...
- (UIView*)generateViewOnMainLoop:(id<STEntityDetail>)entity
    ...
    if (entity && entity.metadata && [entity.metadata count] > 0) {
        ...
        STMetadataContainerView* view = [[STMetadataContainerView alloc] initWithDelegate:main andFrame:CGRectMake(15, 0, 290, 0)];
        ...
        for (NSInteger i = 0; i < [entity.metadata count]; i++) {
            ...
            if (i + 1 != [entity.metadata count] ) {
                UIImageView* bar = [[[UIImageView alloc] initWithImage:image] autorelease];
        ...
        view.gradient = [Util addGradientToLayer:view.layer withColors:[UIColor stampedLightGradient] vertical:YES];
