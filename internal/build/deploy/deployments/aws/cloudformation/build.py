...
import cloudformation, convert, pickle, string, sys, utils
from optparse import OptionParser

g_securityGroups = set()
...
def addInstance(template, instance):
    userData  = buildUserData(instance)
    roleTitle = string.joinfields((role for role in instance.roles), '')
    securityGroup = "%sSecurityGroup" % roleTitle
    
    if 'db' in instance.roles:
        subname = instance.name
        name = "Database"
        amiSize = DATABASE_SIZE
        amiRegion = DATABASE_REGION
        amiSoftware = DATABASE_OS
        amiEBS = DATABASE_EBS
    else:
        name = 'WebServer'
        # NOTE: this assumes only web server instance!
        subname = name
        amiSize = WEBSERVER_SIZE
        amiRegion = WEBSERVER_REGION
        amiSoftware = WEBSERVER_OS
        amiEBS = WEBSERVER_EBS
    
    instanceType = '%sInstanceType' % name
    template.Resources.add('Ec2%sInstance' % subname,
        Type='AWS::EC2::Instance',
        Properties={'SecurityGroups': [{'Ref': securityGroup}],'ImageId': {'Ref': 'ImageId'},
                    'KeyName': {'Ref': 'KeyName'},
                    'ImageId': cloudformation.GetAMI(size=amiSize, 
                                                     region=amiRegion, 
                                                     software=amiSoftware, 
                                                     ebs=amiEBS),
                    'InstanceType': { 'Ref' : instanceType },
                    'Tags': [
                            {'Key': 'stamped:family', 'Value': name},
                            {'Key': 'stamped:server:role', 'Value': name }],
                    'UserData': {'Fn::Base64': {'Fn::Join': ["", userData]}}
        })
    
    if securityGroup not in g_securityGroups:
        # only allow SSH access by default
        access = [
            { # SSH access
                'IpProtocol': 'tcp',
                'FromPort': '22',
        ...
        if 'db' in instance.roles:
            port = str(instance.mongodb.port)
            access.append({ # MongoDB access
                'IpProtocol': 'tcp', 
                'FromPort': port, 
                'ToPort': port, 
                'CidrIp': '0.0.0.0/0'
        ...
        template.Resources.add('%sSecurityGroup' % roleTitle,
            Type='AWS::EC2::SecurityGroup',
            Properties={
                'GroupDescription': 'Enable access to stuff',
                'SecurityGroupIngress': access
            })

def buildTemplate(instances):
    ###############################################################################
    ## CLOUDFORMATION TEMPLATE
    
    template = cloudformation.Template()
    template.Description = 'Stamped CloudFormation Script (v1)'
    
    ## Outputs
    template.Outputs.add('InstanceId',
                  Description='InstanceId of the EC2 WebServer instance',
                  Value={'Ref': 'Ec2WebServerInstance'})
    
    template.Outputs.add('AZ',
                  Description='Availability Zone of the EC2 WebServer instance',
                  Value={'Fn::GetAtt': ['Ec2WebServerInstance', 'AvailabilityZone']})
    
    ## Parameters
    template.Parameters.add('WebServerInstanceType',
                     Description='Type of EC2 instance to launch for web servers',
                     Type='String',
                     Default='t1.micro')
    template.Parameters.add('DatabaseInstanceType',
                     Description='Type of EC2 instance to launch for DB servers',
                     Type='String',
                     Default='t1.micro')
    template.Parameters.add('KeyName',
    ...
    for instance in instances:
        instance = utils.AttributeDict(instance)
        addInstance(template, instance)
    
    # The wait handlers should be named so that they are run in alphabetical order.
    template.Resources.add('RunInitWaitHandle',
        Type='AWS::CloudFormation::WaitConditionHandle',
        Properties={})
    
    template.Resources.add('RunInit',
    ...
    template.Resources.add('CreateWebServerInstance',
        Type='AWS::CloudFormation::WaitCondition',
        DependsOn='Ec2WebServerInstance',
        Properties={
