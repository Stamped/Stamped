...
static float MIN_TREE_SIZE  = 0.9f; //(FULL_SCREEN ? 1.5f : 0.9f);
...
class Particle {
    ...
    // Num frames until this particle will branch next
    int _length;
    ...
    Particle(Particle trunk, boolean sign) {
        this(trunk._x, trunk._y, trunk._theta + (sign ? 1 : -1) * 
             ...
             trunk._size + random(0.0, trunk._size * 0.5));
    }
    ...
    boolean update() {
        ...
        this._x += this._speed * cos(this._theta);
        this._y += this._speed * sin(this._theta);
