...
/* @pjs preload="/assets/img/processing/stamped_mask.png"; */
...
void setup() {
    ...
    mask = loadImage("/assets/img/processing/stamped_mask.png");
    ...
    stops = new Stop[1];
    stops[0] = new Stop(2.0, color(255, 0, 0, 255), color(0, 255, 0, 255));
}
...
class Body {
    ...
    Body(float x, float y, float mass) {
        ...
        //this._image  = createImage(this._size, this._size, ARGB);
        //this._image.mask(mask);
        
        //this._image = this.get_gradient_image(int(this._size), int(this._size), stops);
    }
    
    float linear_gradient(float[] start, float[] stop, float start_offset, float stop_offset, int index, float offset) {
        return (start[index] + ((offset - start_offset) / (stop_offset - start_offset) * (stop[index] - start[index])));
    }
    
    PImage get_gradient_image(int w, int h, Stop[] s) {
        ...
        for (int y = 0; y < h; y++) {
            ...
            for (int x = 0; x < w; x++) {
                ...
                /*for (int i = 0; i < s.length(); i++) {
                    ...
                    if (dv < stop.offset) {
                        //float r = this.linear_gradient(stop.start, stop.end, start_offset, 2.0, 0, dv);
                        //float g = this.linear_gradient(stop.start, stop.end, start_offset, 2.0, 0, dv);
                        //float b = this.linear_gradient(stop.start, stop.end, start_offset, 2.0, 0, dv);
                        
                        //c = color(r, g, b, 255);
                        c = color(0, 0, 255, 255);
                    } else {
                        start_offset = stop.offset;
                    }
                }*/
    ...
    void draw() {
        ...
        //image(this._image, x, y, this._size, this._size);
        //image(mask, x, y, this._size, this._size);
