    ...
    for (exception, code, kind, msg) in exceptions:
        ...
        if libs.ec2_utils.is_ec2():
            try:
                ...
                ### RESTRUCTURE TODO: node_name in libs.ec2_utils?
...
def stamped_http_api_request(requires_auth=True, 
                            ...
                            exceptions=None):
    """
        stamped_http_api_request is Stamped main HTTP API function decorator, taking 
        care of oauth, client, and request validation, optionally parsing the input 
        GET or POST data against a target form, logging, and error handling, 
        including catching all top-level exceptions and converting them to their 
        HTTP error equivalents. stamped_http_api_request also handles responding 
    ...
    def decorator(fn):
        ...
        def wrapper(request, *args, **kwargs):
            try:
                ...
                params = {}
                params['auth_user_id'], params['auth_client_id'] = checkOAuthWithRequest(request, required=requires_auth)
                ...
                if parse_request:
                    ...
                    if upload is not None:
                        if form is None:
                            raise Exception("ERROR: handleHTTPRequest requires form if upload is provided")
                        
                        params['form']   = parseFileUpload(form(), request, upload, **parse_kwargs)
                    elif form is not None:
                        params['form']   = parseRequest(form(), request, **parse_kwargs)
                    else:
                        params['form']   = parseRequest(None, request, **parse_kwargs)
