...
def run(request, auth_user_id, form, **kwargs):
    activity = activity_api.getActivity(auth_user_id, form.scope, limit=form.limit, offset=form.offset)

    result = []
    for item in activity:
        try:
            result.append(HTTPActivity().importEnrichedActivity(item).data_export())
