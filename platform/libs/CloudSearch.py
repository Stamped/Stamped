...
def connect_cloudsearch(aws_access_key_id=None, aws_secret_access_key=None, **kwargs):
    ...
    """
    return CloudSearchConnection(aws_access_key_id, aws_secret_access_key, **kwargs)
...
class Domain(object):
    ...
    def create_rank_expression(self, name, expression):
        return self.connection.create_rank_expression(self.name, name, expression)
    ...
    def allow_ip(self, ip):
        ...
        if not policies.get('options'):
            ...
            return self.connection.update_service_access_policies(self.name, json.dumps(p))
        else:
            noop = True
            ...
            if not noop:
                self.connection.update_service_access_policies(self.name, json.dumps(policies))
    ...
    def disallow_ip(self, ip):
        ...
        for statement in policies['Statement']:
            if ip in statement['Condition']['IpAddress']['aws:SourceIp']:
                ...
                boto.log.warn('IpAddress %s already not allowed for resource %s. Noop.' % 
                              (ip, statement['Resource']))
...
class SearchConnection(object):
    ...
    def _build_query(self, q=None, bq=None, rank=[], return_fields=[], size=10,
                     start=0, facet=[], facet_constraints={}, facet_sort={}, facet_top_n={},
                     t={}):
    ...
    def search(self, q=None, bq=None, rank=[], return_fields=[], size=10,
        ...
        query = self._build_query(q=q, bq=bq, rank=rank, return_fields=return_fields,
                                  size=size, start=start, facet=facet,
                                  facet_constraints=facet_constraints, facet_sort=facet_sort,
                                  facet_top_n=facet_top_n, t=t)
...
class DocumentServiceConnection(object):
    ...
    def commit(self):
        ...
        if ': null' in sdf:
            log.error('null value in sdf detected. This will probably raise a 500 error.')
...
class CommitResponse(object):
    ...
    def __init__(self, response, doc_service, sdf):
        ...
        try:
            ...
            raise boto.exception.BotoServerError(self.response.status_code, '', body=self.content)
    ...
    def _check_num_ops(self, type_, response_num):
        ...
        if response_num != commit_num:
            log.error('Incorrect number of {}s returned. Commit: {} '
                'Respose: {}'.format(type_, commit_num, response_num))
...
class CloudSearchConnection(AWSQueryConnection):
    ...
    def create_index_field(self, domain_name, field_name, field_type,
        ...
        if field_type not in ['literal', 'uint', 'text']:
            raise Exception('Invalid field type `%s`. Must be literal, uint, or text.' % field_type)
